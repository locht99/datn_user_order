<template>
  <section style="height: 568px; max-height: 568px"
    ...
    <div v-if="listTransaction.length <= 0" class="flex justify-center items-center" style="height: 400px;">
                <p class="font-semibold text-center px-3">
                  Bạn chưa có giao dịch nào gần đây.
                </p>
            </div>
    <Pagination v-if="dataPagination.last_page > 1" :pagination="dataPagination" :offset="5"
      @pagination-change-page="getTransactionByUser"></Pagination>
  </section>
</template>
